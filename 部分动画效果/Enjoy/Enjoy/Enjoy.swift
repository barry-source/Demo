...
class Enjoy: UIControl, UIGestureRecognizerDelegate {
    ...
    public var imageState: ImageState = .dislike {
        didSet {
            if imageState == .like {
                ...
                imageView.image = UIImage(named: "icon_home_like_before")
    ...
    @objc func longGestureDidClick(_ gesture: UILongPressGestureRecognizer) {
        if gesture.state == .began {
            ...
            if self.bounds.contains(location) {
                ...
                if imageState == .like {
                    imageView.image = UIImage(named: "icon_home_like_after")
                } else {
                    imageView.image = UIImage(named: "icon_home_like_before")
                    imageView.tintColor = .red
                }
    ...
    lazy var circleView: CircleView = {
        ...
        circleView.callback = { [weak self] in
            guard let self = self else { return }
            self.imageView.image = UIImage(named: "icon_home_like_after")
